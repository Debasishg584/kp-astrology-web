...
# --- ML Integration ---
try:
    from src.core_transit import DivineTransitEngine # type: ignore
...
class DataConverter:
    ...
    def get_absolute_longitude(planet_data):
        """Fallback for absolute longitude used by ML prep. Prioritizes precise float key."""
        long_val = planet_data.get('long')
        if long_val is not None and isinstance(long_val, (int, float)):
             deg = float(long_val)
        else:
    ...
    def parse_window_date(window_str: str) -> Optional[datetime]:
        ...
        # Precise split to avoid breaking DD-MM-YYYY date separators
...
class MarriageWidowedForensics:
    ...
    def __init__(self, chart_data: dict):
        ...
        self.dob = DataConverter.parse_robust_date(dob_raw.split(' ')[0]) or datetime(1900, 1, 1)
        ...
        for md_raw in dasa_table:
            ...
            for ad_raw in md.get('sub_periods', []):
                ...
                for pd_raw in ad.get('sub_periods', []):
                    ...
                    event_type = "DIVORCE"
                    # Refined Widowhood Logic: Maraka/Badhaka for Spouse (Natal 2, 8) + Separation (Natal 6)
                    if pd_h.intersection({2, 8}) and pd_h.intersection({6}) and pd_h.intersection({8}):
                        event_type = "WIDOWHOOD"
                    elif pd_h.intersection({1, 6, 12}):
                        event_type = "DIVORCE"
        ...
        for md_raw in dasa_table:
            ...
            for ad_raw in md.get('sub_periods', []):
                ...
                for pd_raw in ad.get('sub_periods', []):
                    ...
                    # Sun Transit Confirmation (V11 Standard)
                    transit_confirmed = False
                    curr_check = pd_start_dt
                    raw_p_end = str(pd.get('end', ''))
                    if not raw_p_end: continue
                    ...
                    if pd_end_dt:
                        max_checks = 500 # Limit scan for UI responsiveness
                        checks = 0
                        # Fertile signs for Sun: Cancer, Scorpio, Pisces
                        fertile_sun_signs = [3, 7, 11] # 0-indexed: Cancer=3, Scorpio=7, Pisces=11
                        while curr_check <= pd_end_dt and checks < max_checks:
                            # Use MarriageWidowedForensics's get_sun_sign if available or inline
                            m = curr_check.month
                            d = curr_check.day
                            sun_sign = 0
                            ...
                            if sun_sign in fertile_sun_signs:
                                transit_confirmed = True
                                break
                            curr_check += timedelta(days=1)
                            checks += 1
...
class TitaniumAI:
    EVENT_INTENT_MAP = {
        ...
        "property": {"houses": [4, 11, 12], "keywords": ["property", "house", "land", "real estate"]},
        "vehicle": {"houses": [4, 11, 12], "keywords": ["vehicle", "car", "bike", "transport"]},
        "divorce": {"houses": [1, 6, 8, 12], "keywords": ["divorce", "separation", "widow", "widowed"]},
        "wealth": {"houses": [2, 6, 11], "keywords": ["wealth", "money", "finance", "gain", "income"]},
        "loss": {"houses": [5, 8, 12], "keywords": ["loss", "expense", "drain"]},
        "business": {"houses": [2, 7, 10, 11], "keywords": ["business", "partnership", "trade", "company"]},
        "loan": {"houses": [6, 11], "keywords": ["loan", "debt", "borrow"]},
        "repay": {"houses": [5, 8, 11], "keywords": ["repay", "repayment", "clear debt"]},
        "hospital": {"houses": [6, 8, 12], "keywords": ["hospital", "surgery", "medical", "treatment"]},
        "court": {"houses": [6, 8, 12], "keywords": ["court", "legal", "litigation", "case"]},
        "life": {"houses": [1, 8], "keywords": ["life", "death", "span", "longevity"]}
    }
    ...
    def __init__(self, root, lang=None):
        from src.translations import get_lang # type: ignore
        self.root = root
        self.language = lang or get_lang()
        self.root.title(t("titanium_window_title"))
        ...
        import builtins
        # Fix: Ensure rich fallback translation is used if primary 'translations.t' is missing or restricted
        if not hasattr(builtins, 't') and ('t' not in globals() or t is None or (hasattr(t, '__name__') and t.__name__ == '<lambda>')):
            globals()['t'] = fallback_t
    ...
    def _prepare_ml_data(self, target_date=None):
        ...
        # Use deepcopy to prevent side-effects from sub-engines mutating nested structures
        data = copy.deepcopy(self.run_chart_data)
    ...
    def _prepare_ml_data_for_item(self, item: dict) -> dict:
        ...
        # Use deepcopy instead of shallow copy to ensure absolute state isolation.
        ml_data: Any = copy.deepcopy(self.run_chart_data)
    ...
    def run_task_async(self, func, *args, callback=None):
        """Run a long-running task in a background thread and call callback when done."""
        def wrapper():
            try:
                result = func(*args)
                if callback:
                ...
                self.log(f"CRITICAL ASYNC ERROR: {e}", "red")
                if callback:
    ...
    def setup_timing_tab(self):
        ...
        # --- Event Timing Modules (15 Specific Items) ---
        self.event_modules = [
            "Business Timing",
            "Child Birth Timing",
            "Court Case Timing",
            "Divorce Widowed Timing",
        ...
        if THEME_AVAILABLE:
            ...
            # Replace Set Target Button with Dropdown (ttk.Combobox)
            dropdown_frame = tk.Frame(btn_row1, bg=card_bg)
            dropdown_frame.pack(side="left", padx=8)
            
            self.timing_dropdown = ttk.Combobox(dropdown_frame, values=self.event_modules, state="readonly", width=25, style="Titanium.TCombobox", font=Fonts.BUTTON)
            self.timing_dropdown.set("Select Event Timing...") # Placeholder
            self.timing_dropdown.pack(pady=5)
            
            ModernButton(btn_row1, text="⚡  Generate", command=lambda: self.run_task_async(self.analyze_context), bg=Colors.BG_HOVER, hover_bg=Colors.GOLD_DARK, fg=Colors.GOLD, width=150, height=40).pack(side="left", padx=8)
        else:
            ...
            self.timing_dropdown = ttk.Combobox(btn_frame, values=self.event_modules, state="readonly", width=22)
            self.timing_dropdown.set("Select Event Timing...")
            self.timing_dropdown.grid(row=0, column=1, padx=8)
            
            tk.Button(btn_frame, text="Generate", bg="#1a1a1a", fg="#00e5ff", command=lambda: self.run_task_async(self.analyze_context), 
    ...
    def read_chart(self):
        try:
            if os.path.exists(DATA_PATH):
                ...
                if hasattr(self.chart_indicator, 'set_status'):
    ...
    def analyze_context(self):
        if not self.chart_data:
            ...
            return
        # Get query from dropdown if available
        if hasattr(self, 'timing_dropdown'):
            selection = self.timing_dropdown.get()
            if selection and selection != "Select Event Timing...":
                self.user_query = selection.lower()
                self.log(f"Target Event: {selection.upper()}", "cyan")

        if not self.user_query:
            self.log("❌ ERROR: Please select an event from the dropdown (e.g., Marriage Timing).", "red")
            return
        ...
        # --- UNIFIED EVENT LOADER (V11) ---
        target_event = None
        event_map = {
            "life": ("event", "life_span.py", "calculate_longevity"),
            "death": ("event", "life_span.py", "calculate_longevity"),
            "span": ("event", "life_span.py", "calculate_longevity"),
            "marriage": ("event", "marriage_timing.py", "generate_full_proof_report"),
            "wedding": ("event", "marriage_timing.py", "generate_full_proof_report"),
            "child": ("event", "child_birth_timing.py", "calculate_timing_report"),
            "birth": ("event", "child_birth_timing.py", "calculate_timing_report"),
            "pregnancy": ("event", "child_birth_timing.py", "calculate_timing_report"),
            "divorce": ("event", "divorce_widowed_timing.py", "calculate_timing_report"),
            "widow": ("event", "divorce_widowed_timing.py", "calculate_timing_report"),
            "separation": ("event", "divorce_widowed_timing.py", "calculate_timing_report"),
            "job loss": ("event", "job_loss_timing.py", "generate_full_proof_report"),
            "job start": ("event", "job_start_timing.py", "generate_full_proof_report"),
            "career": ("event", "job_start_timing.py", "generate_full_proof_report"),
            "wealth gain": ("event", "wealth_gain_timing.py", "generate_full_proof_report"),
            "wealth loss": ("event", "wealth_loss_timing.py", "generate_full_proof_report"),
            "income": ("event", "wealth_gain_timing.py", "generate_full_proof_report"),
            "money": ("event", "wealth_gain_timing.py", "generate_full_proof_report"),
            "property": ("event", "property_purchase_timing.py", "generate_full_proof_report"),
            "vehicle": ("event", "vehicle_purchase.py", "generate_full_proof_report"),
            "court": ("event", "court_case_timing.py", "generate_full_proof_report"),
            "litigation": ("event", "court_case_timing.py", "generate_full_proof_report"),
            "hospital": ("event", "hospitalization_timing.py", "generate_full_proof_report"),
            "repay": ("event", "loan_repay_timing.py", "generate_full_proof_report"),
            "loan": ("event", "loan_repay_timing.py", "generate_full_proof_report"),
            "business": ("event", "wealth_gain_timing.py", "generate_full_proof_report")
        }
        ...
        if target_event:
            folder, filename, func_name = target_event
            self.log(f">>> HANDING OVER TO: {filename}", "gold")
            loader = self.dynamic_load_module(folder, filename, func_name)
            if loader:
                try:
                    import inspect
                    sig = inspect.signature(loader)
                    # If it's a class (Forensics), it might need instantiation
                    # But our dynamic_load_module usually returns a method or connect_titanium
                    
                    if len(sig.parameters) == 1:
                        report = loader(current_chart_data)
                    else:
                        # V11 standard: connect_titanium(chart_data, lang)
                        report = loader(current_chart_data, self.language)
                    
                    # Unified Result Capture
                    event_key = filename.replace('_timing.py', '').replace('.py', '').title().replace('_', ' ')
                    self.gross_prediction_data[event_key] = report.get("Event_Windows", report.get("Confirmed_Event_Windows", []))
                    if "error" in report:
                        self.log(f"❌ Analysis Error: {report['error']}", "red")
                    else:
                        self.log(f"✅ Analysis Complete: Found {len(self.gross_prediction_data[event_key])} windows.")
                    
                    # Flush internal logs if available
                    if 'narrative' in report: self.log(report['narrative'])
                except Exception as e:
                    self.log(f"❌ Bridge Execution Error in {filename}: {e}", "red")
            else:
                 self.log(f"❌ Failed to load event engine for {filename}", "red")
    ...
    def _run_ml_candidate_verification(self):
        ...
        q = self.user_query.upper()
        event_type = None
        
        # Strict mapping to available ML models
        if "CHILD" in q: event_type = "child_birth"
        elif "MARRIAGE" in q: event_type = "marriage"
        elif "JOB LOSS" in q: event_type = None
        elif "JOB" in q: event_type = "job_start"
        elif "LIFE" in q or "DEATH" in q: event_type = "death"
        
        allowed_models = ["death", "child_birth", "marriage", "job_start"]
        if not event_type or event_type not in allowed_models:
             # Don't log ML verification if no model matches the event
             return
                ...
                if start_dt:
                    try:
                        ...
                        if engine_ref:
                            ...
                            # CRITICAL FIX: Defined raw_prob from ML engine call
                            raw_prob = engine_ref.predict_probability(event_type, ml_data)
                            if raw_prob is None or str(raw_prob).lower() == 'nan':
                                self.log(f"⚠️ ML returned invalid/None probability. Discarding candidate.", "yellow")
                                continue
    ...
    def setup_general_tab(self):
        ...
        # --- General Analysis Modules (63 Specific Items) ---
        self.general_modules = [
            "Bank Balance Predict", "Business Partnership Predict", "Business Service Predict",
            "Career Direction Predict", "Children Predict", "Couple Compatibility",
            "Courage Initiation Predict Advanced", "Court Case Predict", "Creativity Intelligence Predict",
            "Death Reason Predict", "Dharma Ethics Predict", "Disease Health Predict",
            "Divorce Widowhood Predict", "Dream Pattern Predict", "Education Predict",
            "Elder Siblings Predict", "Election Predict", "Enemies Predict",
            "Esoteric Knowledge Predict", "Family Relatives Predict", "Fear Subconscious Predict",
            "Food Eating Predict Advanced", "Foreign Travel Predict", "Hidden Matters Predict",
            "House Residence Predict", "Inheritance Predict", "Insurance Predict",
            "Jail Imprisonment Predict", "Job Interview Predict", "Karmic Indicators Engine",
            "KP Appearance", "Loan Debt Predict", "Loan Repayment Predict",
            "Losses Expenses Predict", "Love Affair Predict", "Love Relationships Predict",
            "Marriage Count Predict", "Marriage Location Predict", "Mother In Law Predict",
            "Mother Psychology Predict", "Past Life Story Generator", "Past Life Signs",
            "Personality Predict", "Post Marriage Predict", "Psychological Predict",
            "Scandal Predict", "Second Child Predict", "Secret Relationship Predict",
            "Self Effort Talent Predict", "Sexual Intensity Predict", "Sexual Life Predict",
            "Skills Hobbies Predict", "Soul Journey Engine", "Speculation Trading Predict",
            "Speech Communication Predict Advanced", "Spiritual Blessings Predict",
            "Spouse Psychology Predict", "Sudden Gain Loss Predict", "Surgery Predict",
            "Teacher Mentor Predict", "Vastu Predict", "Vehicle Purchase Predict",
            "Writing Media Predict"
        ]
        ...
        if THEME_AVAILABLE:
            ...
            # Replace Set Query Button with Dropdown (ttk.Combobox)
            dropdown_frame = tk.Frame(btn_row1, bg=card_bg)
            dropdown_frame.pack(side="left", padx=10)
            self.gen_dropdown = ttk.Combobox(dropdown_frame, values=self.general_modules, state="readonly", width=35, style="Titanium.TCombobox", font=Fonts.BUTTON)
            self.gen_dropdown.set("Select Analysis Topic...")
            self.gen_dropdown.pack(pady=5)
            ...
            btn_row2.pack()
            self.btn_gen_generate = ModernButton(btn_row2, text="⚡  Generate Info", command=lambda: self.run_task_async(self.gen_generate_info), bg=Colors.BG_MEDIUM, hover_bg=Colors.BG_HOVER, fg=Colors.TEXT_MUTED, width=170, height=40)
            ...
            tk.Button(btn_frame, text="Read Data", bg="#1a1a1a", fg="#FFD700", command=self.gen_read_data, 
                      font=("Segoe UI", 10, "bold"), width=15, height=2, bd=1, relief="flat").grid(row=0, column=0, padx=8)
            
            self.gen_dropdown = ttk.Combobox(btn_frame, values=self.general_modules, state="readonly", width=30)
            self.gen_dropdown.set("Select Analysis Topic...")
            self.gen_dropdown.grid(row=0, column=1, padx=8)
            
            tk.Button(btn_frame, text="Check Possibility", bg="#4a148c", fg="white", command=self.gen_check_possibility, 
                      font=("Segoe UI", 10, "bold"), width=15, height=2, bd=1, relief="flat").grid(row=0, column=2, padx=8)
            
            self.btn_gen_generate = tk.Button(btn_frame, text="Generate Info", bg="#1a1a1a", fg="#FFD700", command=lambda: self.run_task_async(self.gen_generate_info), state="disabled", 
                                              font=("Segoe UI", 10, "bold"), width=15, height=2, bd=1, relief="flat")
            self.btn_gen_generate.grid(row=1, column=0, padx=8, pady=10)
            
            self.btn_gen_result = tk.Button(btn_frame, text="Show Result", bg="#2e7d32", fg="white", command=self.gen_show_result, state="disabled", 
                                            font=("Segoe UI", 10, "bold"), width=15, height=2, bd=1, relief="flat")
            self.btn_gen_result.grid(row=1, column=1, padx=8, pady=10)
    ...
    def gen_check_possibility(self):
        # Get query from dropdown if available
        if hasattr(self, 'gen_dropdown'):
            selection = self.gen_dropdown.get()
            if selection and selection != "Select Analysis Topic...":
                self.gen_query = selection.lower().strip()
                self.gen_log(f"\n🔍 TARGET TOPIC: {selection.upper()}", "title")
            else:
                self.gen_query = ""
        
        if not self.gen_query or not self.chart_data: 
            if not self.chart_data:
                messagebox.showwarning("Warning", "Please Load Data First.")
            else:
                messagebox.showwarning("Warning", "Please select an analysis topic from the dropdown.")
            return

        self.gen_log(f"Analyzing Chart Promise for {self.gen_query.upper()}...", "title")
        
        # Real logic: Look for keyword in significator mapping
        relevant_houses = []
        ...
        elif "child" in q: relevant_houses = [2, 5, 11]
        elif "money" in q or "wealth" in q or "balance" in q: relevant_houses = [2, 11]
    ...
    def dynamic_load_module(self, folder, filename, primary_function, log_func=None):
        """Unified V11 Dynamic Module Loader"""
        if log_func is None: log_func = self.log
        
        # Use the global project_root defined at top of file
        module_path = os.path.join(project_root, 'prediction', folder, filename)
        
        if not os.path.exists(module_path):
            log_func(f"❌ Module not found at: {module_path}", "red")
            return None

        try:
            # Unique module name to prevent cache collisions
            unique_name = f"bridge_{folder}_{filename.replace('.', '_')}"
            spec = importlib.util.spec_from_file_location(unique_name, module_path)
            if spec and spec.loader:
                module = importlib.util.module_from_spec(spec)
                
                # --- FIX: Set Package Context for Relative Imports ---
                # This ensures `from .transit_utils import ...` works correctly
                module.__package__ = f"prediction.{folder}"
                
                sys.modules[spec.name] = module
                spec.loader.exec_module(module)
                
                # Try connect_titanium first (V11 Standard)
                if hasattr(module, "connect_titanium"):
                    return getattr(module, "connect_titanium")
                # Fallback to primary function
                if hasattr(module, primary_function):
                    return getattr(module, primary_function)
                # Last resort: search for report functions
                for name, obj in inspect.getmembers(module):
                    if inspect.isfunction(obj):
                        if (name.startswith("get_") and name.endswith("_report")) or name.startswith("analyze_"):
                            return obj
                
            log_func(f"❌ Could not resolve entry point in {filename}.", "red")
            return None
        except Exception as e:
            log_func(f"❌ Load Error [{filename}]: {e}", "red")
            return None
    ...
    def gen_generate_info(self):
        ...
        module_map = {
            ...
            "money": ("bank_balance_predict.py", "analyze_financial_dna"),
            ...
            "spirituality": ("spiritual_blessings_predict.py", "analyze_spiritual_blessings"),
            ...
            # Full 63 Items (Standard Dropdown Mapping)
            "bank balance predict": ("bank_balance_predict.py", "analyze_financial_dna"),
            "business partnership predict": ("business_partnership_predict.py", "analyze_business_partnership"),
            "business service predict": ("business_service_predict.py", "analyze_business_service"),
            "career direction predict": ("career_direction_predict.py", "get_career_direction_report"),
            "children predict": ("children_predict.py", "get_children_report"),
            "couple compatibility": ("couple_compatibility.py", "connect_titanium"),
            "courage initiation predict advanced": ("courage_initiation_predict_advanced.py", "connect_titanium"),
            "court case predict": ("court_case_predict.py", "get_court_case_report"),
            "creativity intelligence predict": ("creativity_intelligence_predict.py", "analyze_creativity_intelligence"),
            "death reason predict": ("death_reason_predict.py", "get_death_reason_report"),
            "dharma ethics predict": ("dharma_ethics_predict.py", "analyze_dharma_ethics"),
            "disease health predict": ("disease_health_predict.py", "get_disease_health_report"),
            "divorce widowhood predict": ("divorce_widowhood_predict.py", "get_divorce_widowhood_report"),
            "dream pattern predict": ("dream_pattern_predict.py", "get_dream_pattern_report"),
            "education predict": ("education_predict.py", "get_education_report"),
            "elder siblings predict": ("elder_siblings_predict.py", "analyze_elder_siblings"),
            "election predict": ("election_predict.py", "analyze_election"),
            "enemies predict": ("enemies_predict.py", "analyze_enemies"),
            "esoteric knowledge predict": ("esoteric_knowledge_predict.py", "analyze_esoteric"),
            "family relatives predict": ("family_relatives_predict.py", "get_family_relatives_report"),
            "fear subconscious predict": ("fear_subconscious_predict.py", "analyze_fears"),
            "food eating predict advanced": ("food_eating_predict_advanced.py", "analyze_food_eating"),
            "foreign travel predict": ("foreign_travel_predict.py", "get_foreign_travel_report"),
            "hidden matters predict": ("hidden_matters_predict.py", "analyze_hidden_matters"),
            "house residence predict": ("house_residence_predict.py", "analyze_house_residence"),
            "inheritance predict": ("inheritance_predict.py", "analyze_inheritance"),
            "insurance predict": ("insurance_predict.py", "analyze_insurance"),
            "jail imprisonment predict": ("jail_imprisonment_predict.py", "analyze_imprisonment"),
            "job interview predict": ("job_interview_predict.py", "analyze_interview"),
            "karmic indicators engine": ("karmic_indicators_engine.py", "get_karmic_report"),
            "kp appearance": ("kp_appearance.py", "connect_titanium"),
            "loan debt predict": ("loan_debt_predict.py", "analyze_loan_debt"),
            "loan repayment predict": ("loan_repayment_predict.py", "analyze_repayment"),
            "losses expenses predict": ("losses_expenses_predict.py", "analyze_losses"),
            "love affair predict": ("love_affair_predict.py", "analyze_love_affair"),
            "love relationships predict": ("love_relationships_predict.py", "analyze_love_relationships"),
            "marriage count predict": ("marriage_count_predict.py", "analyze_marriage_count"),
            "marriage location predict": ("marriage_location_predict.py", "analyze_marriage_location"),
            "mother in law predict": ("mother_in_law_predict.py", "analyze_mother_in_law"),
            "mother psychology predict": ("mother_psychology_predict.py", "analyze_mother_psychology"),
            "past life story generator": ("Past_Life_Story_Generator.py", "connect_titanium"),
            "past life signs": ("past_life_signs.py", "analyze_sign_placement"),
            "personality predict": ("personality_predict.py", "analyze_personality"),
            "post marriage predict": ("post_marriage_predict.py", "analyze_post_marriage_family"),
            "psychological predict": ("psychological_predict.py", "analyze_psychology"),
            "scandal predict": ("scandal_predict.py", "analyze_scandal"),
            "second child predict": ("second_child_predict.py", "analyze_second_child"),
            "secret relationship predict": ("secret_relationship_predict.py", "analyze_secret_relationship"),
            "self effort talent predict": ("self_effort_talent_predict.py", "analyze_self_effort"),
            "sexual intensity predict": ("sexual_intensity_predict.py", "analyze_sexual_intensity"),
            "sexual life predict": ("sexual_life_predict.py", "analyze_sexual_life"),
            "skills hobbies predict": ("skills_hobbies_predict.py", "analyze_skills"),
            "soul journey engine": ("soul_journey_engine.py", "get_soul_report"),
            "speculation trading predict": ("speculation_trading_predict.py", "analyze_speculation"),
            "speech communication predict advanced": ("speech_communication_predict_advanced.py", "analyze_speech"),
            "spiritual blessings predict": ("spiritual_blessings_predict.py", "analyze_spiritual_blessings"),
            "spouse psychology predict": ("spouse_psychology_predict.py", "analyze_spouse_psychology"),
            "sudden gain loss predict": ("sudden_gain_loss_predict.py", "analyze_sudden_gain_loss"),
            "surgery predict": ("surgery_predict.py", "analyze_surgery"),
            "teacher mentor predict": ("teacher_mentor_predict.py", "analyze_teacher_mentor"),
            "vastu predict": ("vastu_predict.py", "analyze_vastu"),
            "vehicle purchase predict": ("vehicle_purchase_predict.py", "analyze_vehicle_purchase"),
            "writing media predict": ("writing_media_predict.py", "connect_titanium")
        }
        ...
        if filename == "Past_Life_Story_Generator.py":
            self.gen_log(f"📖 Invoking Full Past Life Story Engine (V3.0)...", "highlight")
            try:
                chart_data_copy = copy.deepcopy(self.chart_data)
                func_to_run = self.dynamic_load_module('general', filename, 'connect_titanium', log_func=self.gen_log)
                if func_to_run:
                    self.generated_report = func_to_run(chart_data_copy, getattr(self, 'language', 'en'))
                    self.gen_log("✅ Akashic Record Generated Successfully.", "success")
                    self.btn_gen_result.config(state="normal")
                else:
                    self.gen_log(f"❌ Could not load Past Life Story Engine.", "fail")
                return
            except Exception as ex:
                self.gen_log(f"❌ Runtime Error in Past Life Story Engine: {ex}", "fail")
                return

        self.gen_log(f"Converting Chart Data for {filename}...", "highlight")
        
        # USE UNIFIED LOADER (V11)
        func_to_run = self.dynamic_load_module('general', filename, primary_function, log_func=self.gen_log)
        ...
        if func_to_run:
            ...
            try:
                ...
                # V11 Standard: (chart_data, lang)
                if len(sig.parameters) == 2 and "chart_data" in param_names:
                     self.generated_report = func_to_run(chart_data_copy, self.language)
                # Legacy V10: (p_data, c_data)
                elif len(sig.parameters) == 2:
                     p_data, c_data = DataConverter.convert_for_engine(chart_data_copy)
                     self.generated_report = func_to_run(p_data, c_data)
                # Single Param: (chart_data)
                elif len(sig.parameters) == 1:
                     self.generated_report = func_to_run(chart_data_copy)
                # No Param
                else:
                     self.generated_report = func_to_run()
                
                if isinstance(self.generated_report, dict):
                    self.gen_log(f"📋 Report status: {self.generated_report.get('status', 'SUCCESS')}", "highlight")
    ...
    def gen_show_result(self):
        if not self.generated_report: return
        self.gen_log("\n" + "="*50)
        self.gen_log(f"🔱 FINAL KP ASTROLOGER REPORT: {self.gen_query.upper()}", "title")
        self.gen_log("="*50 + "\n")
        
        if isinstance(self.generated_report, str):
            self.gen_log(self.generated_report)
        
        elif isinstance(self.generated_report, dict):
            report_dict = self.generated_report
            # Priority 1: Narrative (Directly accessible)
            if "narrative" in report_dict:
                self.gen_log(str(report_dict["narrative"]))
            # Priority 2: Report -> Narrative (V11 Standard)
            elif "report" in report_dict and isinstance(report_dict["report"], dict) and "narrative" in report_dict["report"]:
                 self.gen_log(str(report_dict["report"]["narrative"]))
            # Priority 3: Report (if it is just text)
            elif "report" in report_dict and isinstance(report_dict["report"], str):
                 self.gen_log(report_dict["report"])
            # Fallback: key-value pairs
            else:
                for key, block in report_dict.items():
                    if isinstance(block, dict) and "name" in block:
                        self.gen_log(f"LEVEL {key}: {block['name']}", "header")
                        self.gen_log(f"   → {block.get('summary', 'No details')}\n")
                    elif key not in ["status", "module", "version"]:
                        self.gen_log(f"{key}: {block}")
        
        elif isinstance(self.generated_report, list):
            for line in self.generated_report:
                if isinstance(line, dict):
                    ...
                    if role and ('CSL' in role or 'Occupant' in role):
                        ...
                        self.gen_log(f"\n{'─'*50}")
                        if 'PRIMARY' in role:
                            self.gen_log(f"🏛️ {planet} (PRIMARY - CSL)", "header")
                        else:
                            self.gen_log(f"🪐 {planet} (SECONDARY - Occupant)", "header")
                        
                        if star_lord and sub_lord:
                            self.gen_log(f"   ⭐ Star: {star_lord} | Sub: {sub_lord}")
                        
                        if 'Direction:' in outcome:
                            direction = outcome.split('Direction: **')[1].split('**')[0] if 'Direction: **' in outcome else ''
                            career = outcome.split('Career Type: **')[1].split('**')[0] if 'Career Type: **' in outcome else ''
                            self.gen_log(f"   🧭 Direction: {direction}", "highlight")
                            self.gen_log(f"   💼 Career: {career}")
                        elif synthesis:
                            self.gen_log(f"   ★ {synthesis.split('→')[-1].strip()}", "highlight")
                    
                    elif item_type == 'DNA':
                        self.gen_log(f"\n🧬 CAREER DNA (House {house})", "header")
                        csl = line.get('CSL', '')
                        analysis = line.get('Analysis', '')
                        if csl: self.gen_log(f"   ⚙️ Controller Planet: {csl}")
                        if analysis: self.gen_log(f"   ★ {analysis}", "highlight")
                        ...
                        synthesis = line.get('Synthesis', '')
                        outcome = line.get('Outcome', '')
                        self.gen_log(f"\n🪐 House {house} - Planet: {planet}", "header")
                        if sub: self.gen_log(f"   ⚙️ Sub Lord: {sub}")
                        if synthesis: self.gen_log(f"   ★ {synthesis}", "highlight")
                        if outcome: self.gen_log(f"   📖 Result: {outcome}")
                    
                    elif category:
                        header = category
                        if isinstance(header, int): header = f"House {header}"
                        ...
                        self.gen_log(f"\n{header}", "header")
                        if controller: self.gen_log(f"   ⚙️ Controller: {controller}")
                        if verdict: self.gen_log(f"   ★ {verdict}", "highlight")
                        if details: self.gen_log(f"   📖 {details}")
                    
                    else:
                        planet = line.get('Planet', line.get('planet', ''))
                        house = line.get('House', line.get('house', ''))
                        outcome = line.get('Outcome', line.get('outcome', ''))
                        synthesis = line.get('Synthesis', line.get('synthesis', ''))
                        
                        if planet or house:
                            self.gen_log(f"\n📍 {planet or 'House ' + str(house)}")
                            if outcome: self.gen_log(f"   {outcome}")
                            elif synthesis: self.gen_log(f"   {synthesis}")
                        else:
                            self.gen_log(f"\n• {str(line)}")
                else:
                    self.gen_log(str(line))
        else:
            self.gen_log(str(self.generated_report))
            
        self.gen_log("\n" + "="*50)
        self.gen_log("Analysis Complete.", "success")
